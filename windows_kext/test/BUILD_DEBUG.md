...
| `Warning` | Warning + Error + Critical (default, minimal) |
...
## Step 6: Test with Playground

```powershell
cd test\drv_test_playground
...
## Step 7: View Driver Logs

### Ring Buffer Logs (Recommended)

These logs come through the `GetLogs` command and are automatically displayed in the playground application when you have verbose logging enabled (see Step 2).

With `LOG_LEVEL = Severity::Debug`, you'll see:
- Connection verdicts
- Packet injection status
- Redirect operations
- Error messages
- Filter updates
...
**Solution**: Stop and wait before recreating:
```powershell
# In playground:
> stop
# Wait 5 seconds
...
**Solution**: Run playground.exe as Administrator.

### No debug output in playground
...
| Feature | Test Signing | Production Signing |
|---------|-------------|-------------------|
| **Cost** | Free | $$$$ (Code signing certificate ~$300/year) |
| **Watermark** | "Test Mode" shown on screen | No watermark |
| **Security** | Reduced (accepts test certs) | Full security |
| **Valid for** | Development/testing only | Public distribution |
| **Requirement** | `TESTSIGNING ON` | No special mode needed |

For local development and testing, **test signing is perfect** and recommended.
